      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <button 
          ...
          className="bg-slate-700 text-white rounded-lg p-3 uppercase hover:opacity-95">
            {loading ? 'Ucitavanje...' : 'Azuriraj'}
        </button>
      </form>
      <div className="flex justify-between mt-5">
        <span onClick={handleDeleteUser} className="text-red-700 cursor-pointer">Obrisite racun</span>
        <span onClick={handleSignOut} className="text-red-700 cursor-pointer">Odjavite se</span>
      </div>
      <p className="text-red-700">{error ? error : ''}</p>
      <p className="text-green-700 mt-5">
        { updateSuccess ? 'Uspjesno azurirani podaci!' : ''}
      </p>
    </div>
  )
}
